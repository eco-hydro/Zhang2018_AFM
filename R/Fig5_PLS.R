...
## 1. check SOS, EOS, LOS trend information
## 2. autocorrelation of EOS, correlation or EOS and SOS
Trend <- FALSE
if (Trend){
  source("R/mainfunc/main_PLS.R", encoding = "utf-8")
  ...
  EOS <- pheno$EOS
  SOS <- pheno$SOS
  Trend.los <- Trend.sos <- Trend.eos <- list()
  n <- nrow(EOS)
  acor <- kcor <- numeric(n)
  lag = 1
  
  for (i in 1:n){
    if (i %% 300 == 0) cat(sprintf("[%d]\n", i))
    x <- EOS[i, ]
    y <- SOS[i, ]
    kcor[[i]] <- cor(x, y)
    acor[[i]] <- acf(x, lag.max = lag, plot = F)$acf[lag+1]
    
    Trend.eos[[i]] <- mkTrend(x)
    Trend.sos[[i]] <- mkTrend(y)
    Trend.los[[i]] <- mkTrend(x - y)
  }
  trend_vars <- c("z0", "pval0", "z", "pval", "slp")
  Trend_eos <- list.rbind(Trend.eos) %>% set_colnames(trend_vars)
  Trend_sos <- list.rbind(Trend.sos) %>% set_colnames(trend_vars)
  Trend_los <- list.rbind(Trend.los) %>% set_colnames(trend_vars)
  
  x <- list(Trend_sos, Trend_eos, Trend_los) %>% lapply(`[[`, "z") %>% list.cbind() %>% set_colnames(c("SOS", "EOS", "LOS"))
  # x <- data.frame(SOS = Trend_sos$z, EOS = Trend_eos$z, LOS = Trend_los$z)
  x <- data.frame(kcor, acor)
  x[abs(x) < brks_z[1]] <- NA
  source('R/mainfunc/main_VSI.R', encoding = 'UTF-8')
  p <- VSI_show(x, brks_z)
  print(p)
  
  cors <- data.frame(EOS_acf = acor, COR = kcor)
  VSI_show(cors, brks_acf)
}
...
# with sos
v <- list()
v[[1]] <- PLS_main(doys, dataIn.pls, Idetrend = TRUE, hSOS = TRUE)
v[[2]] <- PLS_main(doys, dataIn.pls, Idetrend = FALSE, hSOS = TRUE)

# no sos
v[[3]] <- PLS_main(doys, dataIn.pls, Idetrend = TRUE, hSOS = FALSE)
v[[4]] <- PLS_main(doys, dataIn.pls, Idetrend = FALSE, hSOS = FALSE)
