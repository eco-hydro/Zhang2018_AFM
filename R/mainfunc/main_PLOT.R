...
alpha = c(0.1, 0.05)
n <- 32
freedom <- n - 2#the number of given variables is 3
Rc_cor = sqrt(qf(1 - alpha, 1, freedom)/(qf(1 - alpha, 1, freedom) + freedom)) %T>% print  # 0.11215
# [1] 0.3232835 0.3808629 0.4869316
# autocorrelation critical values
Rc_acf = qnorm(1 - alpha/2)/sqrt(n)
Z = qnorm(1 - alpha/2)

brks <- brks_z <- c(qnorm(1 - alpha/2), 20)
...
# gridClip@data <- x
# p <- llply(1:3, function(i) spplot(gridClip[, i]))
# p2 <- do.call(c, c(p, list(layout = c(2, 2)))) 
# update(p2, as.table = T, colorkey = F)
