...
## plot to check
ggplot(subset(x, method == "Mean"), aes(year, doys, color = cluster)) + 
...
plsr_fit = plsreg1(X, scale(Y), comps=comps, crosval=T); plsr_fit$Q2
...
info_list <- foreach(doy_cluster=doys, metes=dataIn.pls, i = icount, .final = .%>% set_names(clusters)) %:%
  foreach(Y=doy_cluster, X=metes, .final = .%>% set_names(methods)) %do%{
    progress$step()
    suppressMessages(valid_pls(X, Y, minVar = 2, method = "VIP"))
  }

info_mean <- llply(info_list, `[[`, "Mean")
info_mean.max <- ldply(info_mean, function(x) subset(x, nvar == x$nvar[which.min(x$PRESS_adj)]), .id = "cluster") %T>% print
# info_df <- melt_list(info_mean, "cluster")
# info_df <- ldply(info_mean, function(x)x, .id = "cluster") %>% {.[with(., order(cluster, PRESS_adj)), ]}

## get plot data
progress <- create_progress_bar("text")
progress$init(length(clusters) * length(methods));

VIP_list <- foreach(doy_cluster=doys, metes=dataIn.pls, i = icount, .final = .%>% set_names(clusters)) %:%
  foreach(Y=doy_cluster, X=metes, .final = .%>% set_names(methods)) %do%{
    progress$step()
    suppressMessages(valid_pls(X, Y, minVar = 2, method = "VIP", plsModel = F))
  }
VIP_df <- llply(VIP_list, `[[`, "Mean")
# VIP <- llply(VIP_df, function(x) data.frame(nvar = rownames(x), x, stringsAsFactors = F))

# writelist_ToXlsx(info_mean, "pls_validInfo_list.xlsx")
# write.xlsx(info_df, "pls_validInfo2.xlsx")
# 
# ## 落叶阔叶林单独进行测试
# source('R/mainfunc/mainfunc_pls.R', encoding = 'UTF-8')
# Y = doys$落叶阔叶林$Mean
# X = dataIn.pls$落叶阔叶林$Mean
# suppressMessages(valid_pls(X, Y, "VIP"))
